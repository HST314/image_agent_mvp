...
- 首个推进失败（无检查点）后可经 jobs 引导路径重启，成功后 failed_step 被清除；
...
def test_bootstrap_restart_after_first_step_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """首个推进失败且尚无检查点：同步 retry 不可用，jobs 引导路径可安全重启。"""
    client = _client(tmp_path, monkeypatch)
    ...
    assert _wait_job(client, first.json()["job_id"])["status"] == "failed"
    ...
    # 修复后重启（前端在已知失败终态轮换新幂等键）：引导路径从任务卡重新执行
